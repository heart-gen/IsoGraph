...
def _compute_accuracy(
    ...
    switch_auc_vae = (
        _auc(np.array(vae_scores), np.array(vae_labels)) if vae_scores else float("nan")
    )
    ...
    config = ExplainConfig(
        vae_attribution=False, vae_fdr_threshold=0.05, vae_percentile_threshold=90.0
    )
    precision_vals = []
...
def _print_row(r: AccuracyResult):
    ...
    print(
        f"    member_auc:        {r.member_auc:.3f}   (|decoded_delta| vs module membership, all genes)"
    )
    ...
    print(
        f"    switch_auc_vae:    {r.switch_auc_vae:.3f}   (|decoded_delta| vs truth_switch, module genes)"
    )
    ...
    if not np.isnan(r.precision_at_filter):
        print(
            f"    precision@filter:  {r.precision_at_filter:.3f}   (true switching among passes_filter=True)"
        )
