...
def test_module_gene_roles_classify_coupled_and_discordant() -> None:
    ...
    module_table = pd.DataFrame(
        {"module_id": ["M000", "M000", "M000"], "gene_id": ["G1", "G2", "G3"]}
    )
    roles = compute_module_gene_roles(
        module_table, feature_scores, pd.DataFrame({"sample_id": samples}), min_abs_r=0.1
    )
